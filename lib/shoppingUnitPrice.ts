...
import { getPackTypeLabel, type AmountUnit, type PackType } from '@/config/shoppingCategories'
...
/** 총 낱개 수 — 개: 구매 수량, 박스: 박스 수 × 1박스당 개수 */
export function getTotalPieces(packType: PackType = 'piece', packCount = 1, unitsPerPack = 1) {
  ...
  if (packType === 'box') return count * normalizeUnitsPerPack(unitsPerPack)
  return count
}
...
export function formatShoppingDetailLine(meta: ShoppingQuantityMeta): string[] {
  ...
  if (perPieceLabel) {
    const showPerPiece =
      packType === 'box' ? unitsPerPack > 1 || packCount > 1 : packCount > 1
