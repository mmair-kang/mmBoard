...
export type AnnualPaymentDraft = AnnualPaymentPayload & {
  key: string
}

export function paymentsToDrafts(
  payments: Array<{
    id: number
    title: string
    month: number
...
export function draftsToPayments(drafts: AnnualPaymentDraft[]): AnnualPaymentPayload[] {
  return drafts
...
type Props = {
  drafts: AnnualPaymentDraft[]
  onChange: (drafts: AnnualPaymentDraft[]) => void
}

export function AnnualPaymentsEditor({ drafts, onChange }: Props) {
  const [dragKey, setDragKey] = useState<string | null>(null)

  const updateDraft = (key: string, patch: Partial<AnnualPaymentDraft>) => {
      ...
      {drafts.map((draft) => (
        ...
        >
          <Stack direction="row" alignItems="center" spacing={0.5} sx={{ mb: 0.75 }}>
            ...
            <Typography sx={{ fontWeight: 800, fontSize: '0.82rem', flex: 1 }}>연납</Typography>
          ...
          <Stack spacing={0.75}>
            ...
            <Stack direction="row" spacing={0.75}>
              <AnnualMonthSelect
                labelId={`month-${draft.key}`}
                value={draft.month}
                onChange={(month) => updateDraft(draft.key, { month })}
              />
              <AnnualDayModeSelect
