...
export function AnnualPaymentOrderDialog({ open, items, onClose, onSave }: Props) {
  const [draft, setDraft] = useState<AnnualPayment[]>(items)
  ...
  return (
    <AppDialog open={open} onClose={onClose} fullWidth maxWidth="xs" slotProps={formDialogSlotProps}>
          ...
          {draft.map((item) => (
            <Box
              ...
              sx={{
                ...
                borderRadius: 1.5,
                border: 1,
                borderColor: dragId === item.id ? 'success.main' : 'divider',
      ...
      <DialogActions sx={{ px: 2, pb: 2 }}>
        ...
        <Button variant="contained" color="success" onClick={() => void handleSave()} size="small" disabled={saving}>
          {saving ? '저장 중…' : '저장'}
        </Button>
      </DialogActions>
    </AppDialog>
